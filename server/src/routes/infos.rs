...
pub async fn stellar_info(State(handler): State<Handler>) -> Result<impl IntoResponse, StatusCode> {
    let stellar = handler
        .client_registry()
        .find_by_name(&ClientName::new("Stellar"))
        .await
        .map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
    let stellar = stellar.ok_or(StatusCode::INTERNAL_SERVER_ERROR)?;
    ...
    let stellar = StellarClient {
        ...
        grant_types: stellar.grant_types.into_iter().map(Into::into).collect(),
        response_types: stellar.response_types.into_iter().map(Into::into).collect(),
        redirect_uris: stellar.redirect_uris.into_iter().map(Into::into).collect(),
        scopes: stellar.scopes.into(),
        contacts: stellar.contact.into_iter().map(Into::into).collect(),
        jwks: stellar
            .jwks
            .clone()
